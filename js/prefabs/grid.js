...
function Grid(_game, w, h, lineC) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        this.gridsRef[i] = _game.add.bitmapData(960, 768);
        this.gridsRef[i].ctx.strokeSylte = lineC;
        this.gridsRef[i].ctx.lineWidth = 1;
        this.gridsSpr[i] = _game.add.tileSprite(0, 0, 960, 768, this.gridsRef[i]);
        if (i !== 0) {
    ...
    this.cells = [];
    for (let i = 0; i < _game.world.width / w; i++) {
        if (!this.cells[i]) {
            this.cells[i] = [];
        }

        for (let j = 0; j < _game.world.height / h; j++) {
            this.cells[i][j] = {
                occupied: null,
                tile: _game.gen.tiles[i][j],
                connect: []
            };
        }
    }
    ...
    this.bmdOverlay = _game.add.bitmapData(960, 768);
    this.bmdSprite = _game.add.sprite(0, 0, this.bmdOverlay);
...
Grid.prototype.makeGrid = function() {
    for (let x = 0; x < 3; x++) {
        //((x / 2) + 1) === 1, 1.5, 2
        for (let i = 0; i < this._game.world.width; i += this.w * ((x / 2) + 1)) {
            this.gridsRef[x].ctx.moveTo(i, 0);
            this.gridsRef[x].ctx.lineTo(i, 768);
        }

        for (let i = 0; i < this._game.world.height; i += this.h * ((x / 2) + 1)) {
...
Grid.prototype.draw = function(xTiles, yTiles, opacity, color) {
    var curW = this.w * this._game.worldScale;
    var curH = this.h * this._game.worldScale;
    this.upperLeftRow = Math.floor(this._game.camera.view.x / curW);
    this.upperLeftColumn = Math.floor(this._game.camera.view.y / curH);
    this.offsetx = this._game.camera.view.x % curW;
    this.offsety = this._game.camera.view.y % curH;
    ...
    this.bmdSprite.x = this._game.camera.x - this.offsetx;
    this.bmdSprite.y = this._game.camera.y - this.offsety;
    ...
    this.index1 = Math.floor((this._game.input.x + this.offsetx) / curW);
    this.index2 = Math.floor((this._game.input.y + this.offsety) / curH);

    //update the grid offsets with the new index positions
    this.xStart = Math.min(Math.max(this.index1 - Math.floor(xTiles / 2), 0),
        (this._game.world.width / curW) - xTiles);
    this.yStart = Math.min(Math.max(this.index2 - Math.floor(yTiles / 2), 0),
        (this._game.world.height / curH) - yTiles);
